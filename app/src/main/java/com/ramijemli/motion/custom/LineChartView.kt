package com.ramijemli.motion.custom
...
class LineChartView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : View(context, attrs, defStyleAttr) {
    ...
    private var offset = dipToPixels(context, 40f)
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        mTopAxisPath.lineTo(width.toFloat() - anchorOffset, offset)

        mBottomAxisPath.moveTo(mAnchorRect.right + anchorOffset, height - offset)
        mBottomAxisPath.lineTo(width.toFloat() - anchorOffset, height - offset)
    ...
    fun drawProgress(elapsed: Float, totalDuration: Float, progress: Float, totalProgress: Float) {
        val top = offset
        val bottom = height - offset
        val x = (elapsed / totalDuration) * (width - mAnchorRect.right - anchorOffset * 2) + mAnchorRect.right + anchorOffset
